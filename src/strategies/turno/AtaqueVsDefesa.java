...
public class AtaqueVsDefesa implements RegraBatalhaStrategy {
    @Override
    public boolean aplica(ModoBatalha m1, ModoBatalha m2) {
        return (m1 == ModoBatalha.ATAQUE && m2 == ModoBatalha.DEFESA) ||
                (m1 == ModoBatalha.DEFESA && m2 == ModoBatalha.ATAQUE);
    }

    @Override
    public void resolver(Jogador j1, CartaJogada c1, Jogador j2, CartaJogada c2) {
        // Ataque sempre será de quem está em modo ATAQUE
        if (c1.getModo() == ModoBatalha.ATAQUE) {
            if (c1.getPoder() > c2.getPoder()) {
                removerCarta(j2, c2.getCarta());
            }
        } else {
            if (c2.getPoder() > c1.getPoder()) {
